...
public class ModifyInfo : BaseInfo
{
	...
	public virtual IdleNumber RawValue { get; protected set; } = (IdleNumber)0;
	...
	public virtual IdleNumber Calculate()
	{
...
namespace RuntimeData
{
	...
	//	}
	//	public AbilityInfo(StatsType _type, IdleNumber _value, StatModeType _mode) : this(_type, _value, (IdleNumber)0, _mode, false, false)
	//	{
	...
	//			relative = 0;
	//		}
	//		BaseValue = RawValue + (inc * relative);

	//		return BaseValue;
	//	}

	//	public void SetValue(int _level = 1)
	//	{
	...
	//			relative = 0;
	//		}
	//		IdleNumber nextValue = RawValue + (inc * relative);
	...
	//public class RandomOptionInfo : BaseInfo
	//{
	//	public  struct InternalOptionInfo
	//	{
	//		public Grade grade;
	//		public int chance;
	//		public int minValueRatio;
	//		public int maxValueRatio;
	//		public InternalOptionInfo(Grade grade, int chance, int min, int max)
	//		{
	//			this.grade = grade;
	//			this.chance = chance;
	...
	//	public int chance => rawData.chance;
	//	public RandomOptionData rawData { get; private set; }

	//	public IdleNumber min;
	//	public IdleNumber max;

	//	System.Random r => RandomLogic.Option;
	//	public List<InternalOptionInfo> chances { get; private set; } = new List<InternalOptionInfo>();
	//	public override void SetRawData<T>(T data)
	//	{
	//		rawData = data as RandomOptionData;
	//		tid = rawData.tid;


	//		min = (IdleNumber)rawData.min;
	//		max = (IdleNumber)rawData.max;

	//		this.chances.Clear();
	//		var chances = PlatformManager.CommonData.RandomGradeChances;
			
	//		for (int i = 0; i < rawData.randomGrade.Count; i++)
	//		{
	//			int prevChance = 0;
	//			if (i - 1 >= 0)
	//			{
	//				prevChance = rawData.randomGrade[i - 1].limit;
	//			}
	//			var randomGrade = rawData.randomGrade[i];
	//			InternalOptionInfo optionInfo = new InternalOptionInfo(randomGrade.grade, chances[i].chance, prevChance, randomGrade.limit);
	//			this.chances.Add(optionInfo);
	//		}

	//		statType = rawData.type;
	//	}

	//	public RuntimeData.AbilityOptionInfo RollDice()
	//	{
	//		int chance = r.Next(RandomLogic.maxChance);
	//		int low = 0;
	//		Grade winGrade = Grade.D;
	//		InternalOptionInfo internalOptionInfo = default;

	//		for (int i = 0; i < chances.Count; i++)
	//		{
	//			var internalChance = chances[i];

	//			if (low <= chance && chance < low + internalChance.chance)
	//			{
	//				winGrade = internalChance.grade;
	//				internalOptionInfo = internalChance;
	//				break;
	//			}
	//			else
	//			{
	...
	//		float ratio = UnityEngine.Random.Range(internalOptionInfo.minValueRatio + 1, (float)internalOptionInfo.maxValueRatio);

	//		var calculatedValue = max * (ratio / 100f);
	//		var result = Mathf.Max(min, calculatedValue);

	//		AbilityOptionInfo abilityOptionInfo = new AbilityOptionInfo();

	//		abilityOptionInfo.UpdateData(winGrade, rawData.type, rawData.modeType,(IdleNumber)result);

	//		return abilityOptionInfo;
	//	}
	//}
	...
	//	public AbilityOptionInfo Clone()
	//	{
	//		AbilityOptionInfo clone = new AbilityOptionInfo();

	//		clone.isLock = isLock;
	//		clone.isUsable = isUsable;
	//		clone.value = value;
	//		clone.grade = grade;
	...
	//	public void UpdateData(Grade grade, StatsType type, StatModeType modeType , IdleNumber value)
	//	{
	//		this.grade = grade;
	//		this.statsType = type;
	//		this.value = value;
	//		this.modeType = modeType;
	//		ability = new AbilityInfo(statsType, value, modeType);
	//	}

	//	public void UpdateData(AbilityOptionInfo info)
	//	{
	//		UpdateData(info.grade, info.statsType, info.modeType, info.value);
	//	}
	...
	//	}
	//	//최초 데이터 세팅시에만
	//	public void SetData(AbilityOptionInfo info )
	//	{
	//		grade = info.grade;
	//		statsType = info.statsType;
	//		value = info.value;
